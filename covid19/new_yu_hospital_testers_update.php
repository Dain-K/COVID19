...
<?php
        ...
        $sql="SELECT * FROM newcovid19db.testers, newcovid19db.citizens WHERE Testhospital= '영대병원' AND  testers.Cssn=citizens.ssn AND Cssn='$Cssn'";

        $ret=mysqli_query($con,$sql);
        if($ret){
            $count=mysqli_num_rows($ret);
            if($count==0){
                echo $_GET['Cssn']."주민번호의 검진자가 없음!"."<br>";
...
<HTML>
    ...
    <BODY>
        <h1>검진자 정보 수정</h1>
        <FORM METHOD="post" ACTION="new_yu_hospital_testers_update_result.php">
